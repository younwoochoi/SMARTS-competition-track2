...
class Policy(BasePolicy):
    ...
    def __init__(self):
        ...
        self.model.eval()
        self.scaler = pickle.load(open(os.path.join(current_path, 'scaler_IL.pkl'), 'rb'))
    ...
    def act(self, obs: Dict[str, Any]):
        ...
        for agent_id, agent_obs in obs.items():
            ...
            current_x, current_y = rotate_points_for_encode(current_x, current_y, current_heading)
            ...
            predicted_dx, predicted_dy = rotate_points_for_decode(actions['dx'].item(), actions['dy'].item(), current_heading)
            predicted_dh = actions['d_heading'].item() / 100
            
            action = (predicted_dx, predicted_dy, predicted_dh)

            target_pose = np.array([action[0] + agent_obs.ego_vehicle_state.position[0], action[1] + agent_obs.ego_vehicle_state.position[1], action[2] +  float(agent_obs.ego_vehicle_state.heading), 0.1])
